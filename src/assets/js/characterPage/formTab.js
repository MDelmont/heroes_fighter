...
export const createFormTab = () => {
  ...
  const formContainer = createElementWithProps("div", {
    className: "form-tab-part",
  });

  inputSearch = createElementWithProps("input", {
    placeholder: "Chercher un nom de héro",
    className: "inpt-filter-name",
  });
...
const filterTable = () => {
  ...
  let characters = datas.results.filter((character) => {
    const matchSearch =
      removeAccents(character.name).toLowerCase().includes(searchValue) ||
      searchValue?.trim()?.length == 0;
    const matchPublisher =
      character.biography.publisher === publisherValue ||
      publisherValue?.trim()?.length == 0;
    const matchRace =
      character.appearance.race === raceValue || raceValue?.trim()?.length == 0;
